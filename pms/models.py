...
class KeyResult(moaredModel):
    ...
    history = moaredAuditLog(bases=[moaredAuditInfo])
...
class EmployeeObjective(moaredModel):
    ...
    history = moaredAuditLog(bases=[moaredAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
    objects = moaredCompanyManager("employee_id__employee_work_info__company_id")
...
class Comment(models.Model):
    ...
    history = moaredAuditLog(excluded_fields=["comment"], bases=[moaredAuditInfo])
    objects = moaredCompanyManager(
...
class QuestionTemplate(moaredModel):
...
class Question(moaredModel):
    ...
    objects = moaredCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(moaredModel):
    ...
    objects = moaredCompanyManager("question_id__template_id__company_id")


class Feedback(moaredModel):
...
class EmployeeBonusPoint(moaredModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
